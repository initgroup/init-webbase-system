(function() {
    ...
    function selectSkin(skinCode) {
        const exists = templates().some((template) => template.code === skinCode);
        if (!exists) return;
    ...
    async function loadSettings() {
        ...
        try {
            ...
            selectedSkin = committedSkin;
            renderTemplates();
            setStatus(error.message || "시스템 설정을 불러오지 못했습니다.", "error");
        }
    }
    ...
    async function saveSettings(event) {
        ...
        try {
            const payload = await Common.api.request("/admin/site-settings", {
                ...
                signal: controller.signal,
                loadingMessage: "홈페이지 스킨을 저장하고 있습니다."
            });
            const data = Common.data.get(payload) || {};
            ...
            renderTemplates();
            setStatus("홈페이지 스킨을 저장했습니다.", "success");
            Common.ui.toast("홈페이지 스킨을 적용했습니다.", "success");
        } catch (error) {
            if (error?.name !== "AbortError") {
                setStatus(error.message || "홈페이지 스킨을 저장하지 못했습니다.", "error");
            }
        }
    }

    window.Pages["admin-site-settings"] = {
        async init(context) {
            root = context.root;
